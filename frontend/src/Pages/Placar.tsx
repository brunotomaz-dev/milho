   ...
   <main className="main-container">
    <section className="form score-card">
      <div className="radio-switch">
        <label htmlFor="scoreSwitchPersonal" className={ `radio-switch__label ${ renderPlayerScore ? "radio-switch__label--selected" : ""}` } onClick={() => {setrenderPlayerScore(true); setrenderGeneralScore(false)} }>
          <input type="radio" name="scoreSwitch" id="scoreSwitchPersonal" className="radio-switch__input" checked={renderPlayerScore} readOnly />
          Meu Placar
        </label>
        <label htmlFor="scoreSwitchGeneral" className={ `radio-switch__label ${ renderGeneralScore ? "radio-switch__label--selected" : ""}` } onClick={() => {setrenderPlayerScore(false); setrenderGeneralScore(true)} }>
          <input type="radio" name="scoreSwitch" id="scoreSwitchGeneral" className="radio-switch__input" checked={renderGeneralScore} readOnly />
          Placar Geral
        </label>
      </div>
      {renderPlayerScore && <PersonalScore data={playerData} />}
      {renderGeneralScore && <GeneralScore playerScore={playerData} generalScore={allPlayersData} />}
