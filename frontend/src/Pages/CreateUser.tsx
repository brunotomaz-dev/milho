  ...
  const handlePasswordConfirmation = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return (
        <section className="form flex-center-column">
          ...
          <label htmlFor="user">
            <p>Usuário</p>
            <input type="text" name='user' value={user} onChange={({ target: { value }}) => setUser(value)} />
          </label>
          <label htmlFor="email">
            <p>Email</p>
          ...
          <label>
            <p>Senha</p>
            <input type="password" value={password} name="password" placeholder="8 letras ou números" onChange={({ target: { value } }) => setPassword(value)} />
          </label>
          <label>
            <p>Confirmar senha</p>
          ...
          <div>
            <button type="button" disabled={isButtonDisabled} onClick={() => navigate('/config')} >Criar</button>
          </div>
        </section>
    )
  }
